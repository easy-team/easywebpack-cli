...
module.exports = class Action extends Logger {
  ...
  install(options) {
    const config = utils.initConfig(this.program, this.config, {
      install: {
        check: true,
  ...
  dev(options) {
    const config = utils.initConfig(this.program, this.config, options);
  ...
  build(env, options) {
    const config = utils.initConfig(this.program, this.config, { env, cliDevtool : options.devtool}, { speed: options.speed });
  ...
  dll(env, options) {
    const config = utils.initConfig(program, this.config, { env, framework: 'dll' }, { dll: true });
    builder.build(config);
  }
   ...
   */
  print(env, options) {
    const config = utils.initConfig(this.program, this.config, { env });
  ...
  add() {
    this.yellow('[add] command not implemented');
  }
};