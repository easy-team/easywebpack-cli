...
module.exports = class Command extends Logger {
  constructor() {
    ...
    this.commands = ['init', 'install', 'dev', 'start', 'build', 'debug', 'test', 'cov', 'print', 'add',
    'server', 'dll', 'zip', 'tar', 'deploy', 'clean', 'open', 'doc', 'kill', 'upgrade'];
  ...
  register(cmd) {
    if (this.commands.some(key => { key === cmd })) {
      this.red(`The command ${cmd} already exists. Please overwrite the command directly.`);
    } else {
