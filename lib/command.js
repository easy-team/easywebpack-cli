...
module.exports = class Command {
  constructor() {
    ...
    this.boilerplate = {};
    this.commands = ['init', 'install', 'dev', 'start', 'build', 'debug',
      'test', 'cov', 'add', 'print', 'server', 'dll', 'zip', 'tar', 'deploy', 
      'clean', 'open', 'kill', 'upgrade', 'puppeteer'];
  ...
  server() {
    this.program
      ...
      .description('static file web http server, default port is 8888, default dist is .')
      .action(options => {
        options.port = options.port || options.parent && options.parent.port;
