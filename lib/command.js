...
const getBoilerplateInfo = (boilerplate, project) => {
  const filterItems = boilerplateModeChoice[boilerplate].filter(item => project === item.value);
  return filterItems.length > 0 ? filterItems[0] : null;
};

const getBaseProjectInfoChoice = () => {
  return [
    {
      type: 'input',
      name: 'name',
...
const getStandardProjectInfoChoice = () => {
  const baseChoices = getBaseProjectInfoChoice();
  ...
  return [].concat(baseChoices).concat(standardChoices);
};
...
exports.init = options => {
  ...
  inquirer.prompt([{
    ...
    const boilerplate = boilerplateAnswer.boilerplate;
    const download = new Download(options);
    if (boilerplateModeChoice[boilerplate]) {
      const boilerplateModeAsk = [{
        type: 'list',
        name: 'project',
        message: 'please choose the boilerplate project mode?',
        choices: boilerplateModeChoice[boilerplate]
      }];
      inquirer.prompt(boilerplateModeAsk).then(boilerplateModeAnswer => {
        const project = boilerplateModeAnswer.project;
        const bilerplateInfo = getBoilerplateInfo(boilerplate, project);
        const projectInfoChoice = getStandardProjectInfoChoice();
        switch (boilerplate) {
      ...
      const projectInfoChoice = boilerplate === 'npm-package-code-template' ? getBaseProjectInfoChoice() : getStandardProjectInfoChoice();
      inquirer.prompt(projectInfoChoice).then(projectInfoAnswer => {
        const boilerplateInfo = { pkgName: boilerplate };
