...
const initPrompt = (cli, options, choices) => {
  inquirer.prompt([{
    loop: false,
    type: 'list',
    name: 'template',
...
const downloadTemplate = (cli, templateInfo, options) => {
  const d = new Download(options, cli);
  co(function* () {
    const pkgName = 'easyjs-awesome-template';
    const absSourceDir = yield d.download(pkgName, templateInfo.name);
    ...
    const filename = options.filename || templateInfo.filename;
