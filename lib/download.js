...
module.exports = class Download extends Logger {
  ...
  *getPackageInfo(pkgName) {
    this.green(`query npm info of ${pkgName}`, 'yellow');
  ...
  *download(pkgName, dir) {
    ...
    this.green(`downloading ${tgzUrl}`, 'yellow');
    ...
    this.green(`extract to ${this.tempDir}`, 'yellow');
  ...
  installDeps(projectDir, info) {
    ...
    if (npm) {
      ...
      const spinner = ora(this.log(`start ${cmd}...`));
      spinner.start()
